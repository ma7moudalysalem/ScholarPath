...
public class SearchScholarshipsQueryHandler
    : IRequestHandler<SearchScholarshipsQuery, PaginatedResponse<ScholarshipListItemDto>>
{
    private readonly IApplicationDbContext _dbContext;

    public SearchScholarshipsQueryHandler(IApplicationDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<PaginatedResponse<ScholarshipListItemDto>> Handle(
        SearchScholarshipsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (request.CurrentUserId.HasValue)
        {
            savedScholarshipIds = (await _dbContext.SavedScholarships
                .AsNoTracking()
                .Where(ss => ss.UserId == request.CurrentUserId.Value)
