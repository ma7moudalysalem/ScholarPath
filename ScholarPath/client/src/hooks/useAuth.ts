...
export function useAuth() {
  ...
  const logout = useCallback(async (logoutEverywhere: boolean = false) => {
    setLoading(true);
    try {
      await authService.logout(logoutEverywhere);
    } catch {
      console.warn('Server logout failed — cleared locally only');
    } finally {
      setLoading(false);
      storeLogout();
      navigate('/');
