...
{
    /// <summary>
    /// PB-015 T-001..T-006 — reporting views the Power BI dashboards bind to via
    /// DirectQuery. Read-only projections over the OLTP tables; no schema change.
    /// Each view is created with CREATE OR ALTER so the migration is re-runnable.
    /// </summary>
    public partial class AddReportingViews_PB015 : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // T-001 — daily acquisition funnel: registrations, onboarding
            // completion, application submissions, application acceptances.
...
LEFT JOIN accepted acc    ON acc.ActivityDate = s.ActivityDate;");
            ...
            // T-002 — acceptance rates per scholarship x field (category) x
            // student country. Rate is accepted / decided (accepted + rejected).
            migrationBuilder.Sql(@"
...
GROUP BY s.Id, s.TitleEn, s.TitleAr, c.NameEn, c.NameAr, u.CountryOfResidence;");
            ...
            // T-003 — daily finance: gross, profit share, payee net and refunds
            // split by revenue stream. Booking amounts are stored in cents,
            // company-review amounts in USD; both normalised to USD here.
            migrationBuilder.Sql(@"
...
FROM review_payments;");
            ...
            // T-004 — per-consultant KPIs: booking outcomes, no-shows, completed
            // revenue and rating. Power BI filters by ConsultantId (RLS column).
            migrationBuilder.Sql(@"
...
WHERE u.IsDeleted = 0;");
            ...
            // T-005 — one row per student summarising their lifecycle: account
            // state, application outcomes and consultant bookings. Power BI
            // filters by StudentId (RLS column).
            migrationBuilder.Sql(@"
...
WHERE u.IsDeleted = 0;");
        ...
        {
            migrationBuilder.Sql("DROP VIEW IF EXISTS dbo.vw_funnel_daily;");
            migrationBuilder.Sql("DROP VIEW IF EXISTS dbo.vw_acceptance_rates;");
            migrationBuilder.Sql("DROP VIEW IF EXISTS dbo.vw_finance_daily;");
            migrationBuilder.Sql("DROP VIEW IF EXISTS dbo.vw_consultant_kpis;");
            migrationBuilder.Sql("DROP VIEW IF EXISTS dbo.vw_student_journey;");
        }
    }
}
