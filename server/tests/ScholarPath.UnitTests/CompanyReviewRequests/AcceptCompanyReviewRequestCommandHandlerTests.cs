...
public class AcceptCompanyReviewRequestCommandHandlerTests
{
    ...
    public async Task Captures_payment_and_transitions_to_under_review_with_10_90_split()
    {
        ...
        var result = await sut.Handle(new AcceptCompanyReviewRequestCommand(request.Id), default);

        result.Should().BeTrue();
        var updated = db.CompanyReviewRequests.Single(r => r.Id == request.Id);
        updated.Status.Should().Be(CompanyReviewRequestStatus.UnderReview);
    ...
    public async Task Notification_failure_does_not_block_status_transition()
    {
        using var db = CompanyReviewRequestTestFixtures.CreateDb();
        var (request, _) = CompanyReviewRequestTestFixtures
            .SeedRequestWithPayment(db, CompanyReviewRequestStatus.Pending, amountCents: 5_000);
        ...
        result.Should().BeTrue();
        db.CompanyReviewRequests.Single().Status
            .Should().Be(CompanyReviewRequestStatus.UnderReview);
    ...
    public async Task Rejects_non_owning_company()
    {
        ...
        var act = () => sut.Handle(new AcceptCompanyReviewRequestCommand(request.Id), default);
