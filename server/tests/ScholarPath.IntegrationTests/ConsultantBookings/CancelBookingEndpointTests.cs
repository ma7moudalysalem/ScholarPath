...
public sealed class CancelBookingEndpointTests : IntegrationTestBase
{
    public CancelBookingEndpointTests(CustomWebApplicationFactory factory)
        : base(factory)
    {
    }

    [Fact]
    public async Task Post_CancelBooking_ReturnsNoContent_AndCancelsRequestedBooking()
    {
        ...
        await ExecuteScopeAsync(async sp =>
        {
            ...
            bookingId = (await sender.Send(requestBooking)).BookingId;
        });
        ...
        var response = await Client.PostAsJsonAsync(
            $"/api/bookings/{bookingId:D}/cancel",
            command);
        ...
        await ExecuteScopeAsync(async sp =>
        {
            ...
            booking.CancelledByUserId.Should().Be(studentId);
            booking.CancellationReason.Should().Be(CancellationReason.StudentCancelledBeforeAcceptance);
