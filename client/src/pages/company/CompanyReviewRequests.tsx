 ...
 *
 * Earnings row shows the locked-in commission and Company share from the
...
export function CompanyReviewRequests() {
  ...
  const query = useQuery<CompanyReviewRequestDto[]>({
    ...
    queryFn: () => companyReviewRequestsApi.listMineAsCompany(),
  });
  ...
  const acceptMut = useMutation({
    mutationFn: (id: string) => companyReviewRequestsApi.accept(id),
  ...
  const rejectMut = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason?: string }) =>
      companyReviewRequestsApi.reject(id, reason),
    onMutate: ({ id }) => setBusyId(id),
    onSettled: () => setBusyId(null),
    onSuccess: () => {
  ...
  const completeMut = useMutation({
    mutationFn: (id: string) => companyReviewRequestsApi.complete(id),
    onMutate: (id) => setBusyId(id),
    onSettled: () => setBusyId(null),
    onSuccess: () => {
    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6">
      <header className="mb-6">
        <h1 className="text-2xl font-semibold tracking-tight text-text-primary">
          {t("payments:reviewRequest.companyTitle")}
        </h1>
        <p className="mt-1 text-sm text-text-secondary">
          {t("payments:reviewRequest.companySubtitle")}
        </p>
      </header>
      ...
      {query.data && query.data.length === 0 && (
        <div className="rounded-2xl border border-border-subtle bg-bg-elevated p-8 text-center text-sm text-text-secondary">
          {t("payments:reviewRequest.companyEmpty")}
        </div>
      )}
